...
export const Route = createFileRoute("/_authenticated/insights/$metric")({
  ...
  head: ({ loaderData }) => {
    const title = loaderData ? `${loaderData.meta.title} insight — DoseRoutine` : "Insight — DoseRoutine";
    return {
      meta: [
        { title },
        {
...
function MetricDetailPage() {
  ...
  const variant: ChartVariant =
    variantChoice && variants?.includes(variantChoice)
      ? variantChoice
      : ((("defaultVariant" in metric && metric.defaultVariant) || variants?.[0] || "area") as ChartVariant);
  ...
  return (
    ...
    <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6">
      ...
      <div role="tablist" aria-label="Insights time window" className="mt-4 flex flex-wrap gap-1.5">
