/**
 * Link to the DoseRoutine community Discord server.
 *
 * Rendered as a plain anchor so the global external-link handler can send it
 * to the system browser inside the native app (Guideline 4.2).
 */

export const DISCORD_INVITE_URL = "https://discord.gg/T8VqmVKsU";

/** Discord's official wordmark glyph, drawn with currentColor. */
...
/** Footer/community entry point for the Discord server. */
export function DiscordLink({ className }: { className?: string }) {
  return (
    <a
      ...
      className={
        className ??
        "tap-target inline-flex items-center gap-2 rounded-full border border-border px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-muted"
      }
    >
      <DiscordIcon className="h-4 w-4 text-[#5865F2]" />
      Join the DoseRoutine Discord
    </a>
  );
}
